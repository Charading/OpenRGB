...
private:
    ...
    std::string version_;
    std::vector<const char*> zone_names_;
    bool dirty_;
    bool dirty_dim_;

    static constexpr int HIDAPI_ALIENWARE_REPORT_SIZE = 34;
    typedef std::array<unsigned char, HIDAPI_ALIENWARE_REPORT_SIZE> HidapiAlienwareReport;
    ...
    bool ModeAction(
        const uint8_t *mode, const uint16_t *duration, const uint16_t *tempo, const RGBColor *color, unsigned amount);
    bool MultiModeAction(
        const uint8_t *mode, const uint16_t *duration, const uint16_t *tempo, const RGBColor *color, unsigned amount);
