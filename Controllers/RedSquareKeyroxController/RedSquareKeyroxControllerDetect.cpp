...
static Controllers DetectRedSquareKeyroxTKL(hid_device_info* info, const std::string& name)
{
    Controllers result;
...
static Controllers DetectRedSquareKeyroxTKLClassic(hid_device_info* info, const std::string& name)
{
    Controllers result;
    hid_device* dev = hid_open_path(info->path);
    if(dev)
