...
MSIMysticLightController::MSIMysticLightController(hid_device *handle, const char *path) : dev(handle)
{
    if (dev)
    {
        loc = path;
...
bool MSIMysticLightController::ReadFwVersion()
{
    ...
    int num = 1;
    unsigned char request[64] = {1u, 176u},
                  response[64];
    std::fill_n(request + 2, sizeof request - 2, 204u);
...
void
MSIMysticLightController::GetMode(ZONE zone, EFFECT &mode, SPEED &speed, BRIGHTNESS &brightness, bool &rainbow_color) {
    ZoneData *zoneData = GetZoneData(zone);
    if (!zoneData)
        return;
