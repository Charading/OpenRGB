...
#define LOGITECH_G_PRO_LIGHTSPEED_PID           0xC088  //Wired connection for Logitech GPRO Wireless
...
|   NB: Note used but preserved for debugging           |
\*-----------------------------------------------------*/
#define LOGITECH_G_UNIFYING1_RECEIVER_PID               0xC52B      // Unifying receiver
#define LOGITECH_G_NANO_RECEIVER_PID                    0xC52F      // Nano receiver advanced
#define LOGITECH_G_G700_RECEIVER_PID                    0xC531      // G700 receiver
#define LOGITECH_G_UNIFYING2_RECEIVER_PID               0xC532      // Unifying receiver
#define LOGITECH_G_G602_RECEIVER_PID                    0xC537      // G602 receiver
...
#define LOGITECH_GPRO_LIGHTSPEED_VIRTUAL_PID            0x4079      //This is the internal PID for the mouse not the receiver
...
{
    usages      temp_usages;
    //wchar_t     *serial     = info->serial_number;
    ...
    if(info->usage == 1)
    {
        while(info)
        {
            //if(info->serial_number == serial)     //Serial is unreliable
            //{
                hid_device* dev = hid_open_path(info->path);
                if(dev)
                {
...
REGISTER_HID_DETECTOR_IP("Logitech G Pro Wired Gaming Mouse",       DetectLogitechWired,                LOGITECH_VID, LOGITECH_G_PRO_LIGHTSPEED_PID,        2, 0xFF00);
