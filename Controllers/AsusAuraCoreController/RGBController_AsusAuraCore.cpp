...
RGBController_AuraCore::RGBController_AuraCore(AuraCoreController* aura_ptr)
{
    ...
    {
        name        = "ASUS Aura [Unknown]";
        vendor      = "ASUS";
        type        = DEVICE_TYPE_UNKNOWN;
        description = "ASUS Aura Core Unknown Device";
    }
}
...
void RGBController_AuraCore::UpdateSingleLED(int led)
{
    ...
    {
        red = static_cast<unsigned char>(RGBGetRValue(colors[led]));
        green = static_cast<unsigned char>(RGBGetGValue(colors[led]));
        blue = static_cast<unsigned char>(RGBGetBValue(colors[led]));
    }
    else if(curr_mode.color_mode == MODE_COLORS_MODE_SPECIFIC)
    {
        red = static_cast<unsigned char>(RGBGetRValue(curr_mode.colors[led]));
        green = static_cast<unsigned char>(RGBGetGValue(curr_mode.colors[led]));
        blue = static_cast<unsigned char>(RGBGetBValue(curr_mode.colors[led]));
    }
