...
void RGBController_HyperXPulsefireDart::UpdateSingleLED(int led)
{
    ...
    if(modes[active_mode].color_mode == MODE_COLORS_PER_LED)
    {
        ...
        if(leds[led].value == HYPERX_PULSEFIRE_DART_LED_SCROLL)
        {
            hyperx->SendDirect(colors[led], HYPERX_PULSEFIRE_DART_LED_SCROLL, modes[active_mode].value, brightness, modes[active_mode].speed);
        }
        else if(leds[led].value == HYPERX_PULSEFIRE_DART_LED_LOGO)
        {
            hyperx->SendDirect(colors[led], HYPERX_PULSEFIRE_DART_LED_LOGO, modes[active_mode].value, brightness, modes[active_mode].speed);
        }
        else
        {
