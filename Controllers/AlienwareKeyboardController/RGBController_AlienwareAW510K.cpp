...
void RGBController_AlienwareAW510K::DeviceUpdateMode()
{
    ...
    switch(modes[active_mode].value)
    {
        ...
        case ALIENWARE_AW510K_MODE_SPECTRUM:
            alienware->SetMode(modes[active_mode].value, modes[active_mode].speed, 0x00, ALIENWARE_AW510K_RANBOW_COLOR_MODE, 0x00, 0x00, 0x00); //spectrum only set mode, speed and colorMode
            break;

        case ALIENWARE_AW510K_MODE_SINGLE_WAVE:{
            int waveDirection = GetAW520K_WaveDirection(modes[active_mode].direction);
            alienware->SetMode(modes[active_mode].value, modes[active_mode].speed, waveDirection, ALIENWARE_AW510K_SINGLE_COLOR_MODE, red, grn, blu); //Wave only set mode, speed, direction and colorMode
        break;}

        case ALIENWARE_AW510K_MODE_RAINBOW_WAVE:{
            int waveDirection = GetAW520K_WaveDirection(modes[active_mode].direction);
            alienware->SetMode(modes[active_mode].value, modes[active_mode].speed, waveDirection, ALIENWARE_AW510K_RANBOW_COLOR_MODE, 0x00, 0x00, 0x00); //Wave only set mode, speed, direction and colorMode
        break;}
