...
void OpenRGBUpdateInfoPage::on_CheckButton_clicked()
{
    ...
    for (int i = 0; i < int(Parsed.size()); i++)
    {
        ...
        if (Parsed[i][3] == GIT_COMMIT_ID)
        {
            /*--------------------------------------------------------------------------*\
            | Listing past the version that is already installed is stupid and redundant |
            | so it stops if it find a match with the current version                    |
            \*--------------------------------------------------------------------------*/
    ...
    {
        /*-------------------------------------------------------------------------------*\
        | If it doesn't find a match then it just checks against the latest commit's date |
        | This will make sure my (CoffeeIsLife's) stupidity doesn't break stuff           |
        \*-------------------------------------------------------------------------------*/

        #ifndef CurrentCommitDate
        /*-------------------------------------------------------------------------------------------------*\
        | Dev builds don't have a commit date so the date that it compares against so I have to set it here |
        \*-------------------------------------------------------------------------------------------------*/
        int CurrentCommitDate = 0;
        #endif

        int DateInt = Parsed[0][0].replace("-","").toInt();

        if ((DateInt > CurrentCommitDate) && (!OpenRGBUpdateInfoPage::OwnedByUpdateButton))
        {
            ui->StatusMessage->show();
            ui->StatusMessage->setText("The current installed version couldn't be found implying that the current installed version is so old that is isn't on the list anymore");
        }
    }
    if (!OpenRGBUpdateInfoPage::OwnedByUpdateButton)
    {
