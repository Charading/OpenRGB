...
void OpenRGBDialog2::UpdateDevicesList()
{
    ...
    for(unsigned int controller_idx = 0; controller_idx < controllers.size(); controller_idx++)
    {
        ...
        if(!found)
        {
            ...
            | Create the tab label                                  |
            \*-----------------------------------------------------*/
            TabLabel* NewTabLabel = new TabLabel(OpenRGBFont::GetCharFromDeviceType(controllers[controller_idx]->type), QString::fromStdString(controllers[controller_idx]->name), (char *)controllers[controller_idx]->name.c_str(), (char *)context);
